...
# Author: Matt Simerson (@msimerson)

VERSION='2.00'

# assure sqlite3, mkdir, chmod, & pfctl are in $PATH
...
is_valid_ipv4() {
    ...
    # is first octet less than 1
    if [ "${octets[0]}" -lt 1 ]; then return 1; fi
    ...
    if [ "$all_255" -eq 1 ]; then return 1; fi
...
is_valid_ip() {
    ...
    if is_valid_ipv4 "$IP"; then return 0; fi
    if is_valid_ipv6 "$IP"; then return 0; fi
...
load_ip_record() {
    ...
    if [ "$ALLOW" -ne 0 ]; then printf "\tand it is allowed\n"; fi
    if [ "$BLOCK" -ne 0 ]; then printf "\tand it is blocked\n"; fi
}
...
is_allowed() {
    if [ "$ALLOW" -eq 0 ]; then return 1; fi
    return 0
}

is_blocked() {
    if [ "$BLOCK" -eq 0 ]; then return 1; fi

    # should we expire old block entries?
    if [ "$EXPIRE_BLOCK_DAYS" -eq 0 ]; then return 0; fi
...
configure_tcpwrappers() {
    ...
    if [ "$is_setup" -eq 1 ]; then return 0; fi
...
do_connect() {
    ...
    if is_allowed; then return 0; fi
    if is_blocked; then return 0; fi

    if [ "$SEEN" -lt 3 ]; then return 0; fi
...
allow_tcpwrappers() {
    if [ ! -e "$TCPD_DENYLIST" ]; then return; fi
...
allow_pf() {
    ...
    $pfctl -q -t "$FIREWALL_TABLE" -T delete "$IP" 2>/dev/null || {
...
block_pf() {
    ...
    $pfctl -q -t "$FIREWALL_TABLE" -T add "$IP" 2>/dev/null || {
